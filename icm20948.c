...
static unsigned char gyro_smpdiv = 10; //sampling rate = 1125/(1+smpdiv), 8bits
static unsigned int  acc_smpdiv  = 10; //sampling rate = 1125/(1+smpdiv), 12 bits
...
void configMPU()
{
	...
	delay(10);
	icm_read_reg(REG_WHO_AM_I,&value,1);
	if (value != 0xEA) is_sensor = 0;
	printf("MPU WIA is 0x%02X\r\n",value);
}
...
void configAK09916()
{
	...
	if (value != 0x09) is_sensor = 0;
	printf("AK WIA is 0x%02X\r\n",value);
